...
public sealed class PdfXfaReaderTests
{
    ...
    [Fact]
    public void PersistsDatasetReplacementInThePdfDocument()
    {
        PdfDocument source = Document(
            "<template><script>preserved()</script></template>",
            "<datasets><data><form><name>Old</name></form></data></datasets>");
        byte[] originalTemplate = PdfXfaReader.Read(source)!.Packets[0].Data.ToArray();

        PdfDocument reopened = PdfDocument.Open(PdfXfaDocumentEditor.ReplaceDatasets(source,
            new PdfFormDataSet
            {
                Fields = [new PdfFormDataField { Name = "form.name", Values = ["Zoë"] }]
            }));
        PdfXfaInfo saved = Assert.IsType<PdfXfaInfo>(PdfXfaReader.Read(reopened));

        Assert.Equal("Zoë", Assert.Single(PdfXfaDatasets.Read(saved).Fields).Values[0]);
        Assert.Equal(originalTemplate, saved.Packets[0].Data.ToArray());
        Assert.True(saved.ContainsScript);
    }
    ...
    [Fact]
    public void PersistsCombinedXdpDatasetReplacementInThePdfDocument()
    {
        const string xdp = "<xdp><template><subform name=\"preserved\"/></template>"
            + "<datasets><data><form><name>Old</name></form></data></datasets>"
            + "<localeSet><locale name=\"en_US\"/></localeSet></xdp>";
        PdfDocument source = CombinedDocument(xdp);

        PdfXfaInfo saved = PdfXfaReader.Read(PdfDocument.Open(
            PdfXfaDocumentEditor.ReplaceDatasets(source, new PdfFormDataSet
            {
                Fields = [new PdfFormDataField { Name = "form.name", Values = ["Ada"] }]
            })))!;
        string savedXdp = Encoding.UTF8.GetString(saved.Packets.Single().Data.Span);

        Assert.Equal("Ada", Assert.Single(PdfXfaDatasets.Read(saved).Fields).Values[0]);
        Assert.Contains("name=\"preserved\"", savedXdp, StringComparison.Ordinal);
        Assert.Contains("name=\"en_US\"", savedXdp, StringComparison.Ordinal);
    }
    ...
    private static PdfDocument CombinedDocument(string xdp)
    {
        string[] objects =
        [
            "<< /Type /Catalog /Pages 2 0 R /AcroForm 4 0 R >>",
            ...
            "<< /Type /Page /Parent 2 0 R /MediaBox [0 0 100 100] >>",
            "<< /XFA 5 0 R >>",
            $"<< /Length {Encoding.UTF8.GetByteCount(xdp)} >>\nstream\n{xdp}\nendstream"
        ];
        var pdf = new StringBuilder("%PDF-1.7\n");
        var offsets = new List<int>();
