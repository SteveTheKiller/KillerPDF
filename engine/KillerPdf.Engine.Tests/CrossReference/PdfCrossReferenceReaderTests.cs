...
public sealed class PdfCrossReferenceReaderTests
{
    ...
    [Fact]
    public void ReadSection_CompatibilityRecoveryFindsNearbyCrossReferenceStream()
    {
        byte[] prefix = "endobj\n"u8.ToArray();
        byte[] stream = XrefStream(
            [0, 0, 0, 255, 255],
    ...
    [Fact]
    public void ReadSection_CompatibilityRecoveryAllowsInUseEntryBeyondTrailerSize()
    {
        byte[] source = Encoding.ASCII.GetBytes(
            "xref\n0 2\n0000000000 65535 f\n0000000010 00000 n\n" +
            "trailer\n<< /Size 1 >>");
    ...
    [Theory]
    [InlineData("0000009999 00000 n")]
    [InlineData("0000000010 65535 n")]
    public void ReadSection_CompatibilityRecoveryIgnoresUnusableInUseEntries(string entry)
    {
        byte[] source = Encoding.ASCII.GetBytes(
            $"xref\n0 2\n0000000000 65535 f\n{entry}\ntrailer\n<< /Size 2 >>");
        ...
        Assert.Equal(PdfCrossReferenceEntryType.Null, recovered[1].Type);
    }
