...
internal sealed record PdfCcittFaxOptions(
    int K, int Columns, int Rows, bool EndOfLine, bool EncodedByteAlign, bool BlackIs1);

internal static class PdfCcittFaxDecoder
{
    private static readonly CodeTable White = new(
        ...
        "010011011:1728 " + AdditionalMakeup);

    private static readonly CodeTable Black = new(
        ...
        "000011001010:26 000011001011:27 000011001100:28 000011001101:29 " +
        "000001101000:30 000001101001:31 000001101010:32 000001101011:33 " +
        "000011010010:34 000011010011:35 000011010100:36 000011010101:37 " +
        "000011010110:38 000011010111:39 000001101100:40 000001101101:41 " +
        "000011011010:42 000011011011:43 000001010100:44 000001010101:45 " +
        "000001010110:46 000001010111:47 000001100100:48 000001100101:49 " +
        "000001010010:50 000001010011:51 000000100100:52 000000110111:53 " +
        "000000111000:54 000000100111:55 000000101000:56 000001011000:57 " +
        "000001011001:58 000000101011:59 000000101100:60 000001011010:61 " +
        ...
        "0000001100100:1664 0000001100101:1728 " + AdditionalMakeup);

    private const string AdditionalMakeup =
        ...
        ReadOnlySpan<byte> encoded, PdfCcittFaxOptions options, int maximumDecodedBytes)
    {
        ...
        try
        {
            stride = checked(options.Columns + 7) / 8;
            length = checked(stride * options.Rows);
        }
        catch (OverflowException ex)
        {
            throw new PdfFilterException("CCITT dimensions exceed the supported range.", ex);
        }
        if (length > maximumDecodedBytes)
            throw new PdfFilterException("Decoded stream exceeds the configured safety limit.");

        byte white = options.BlackIs1 ? (byte)0 : (byte)255;
    ...
    private sealed class CodeTable
    {
        ...
        internal CodeTable(string specification)
        {
            foreach (string entry in specification.Split(' ', StringSplitOptions.RemoveEmptyEntries))
            {
                ...
                int code = 0;
                foreach (char bit in parts[0]) code = code << 1 | bit - '0';
        ...
        internal int Read(ref BitReader bits)
        {
            ...
            for (int length = 1; length <= _maximumLength; length++)
            {
                ...
                if (_codes.TryGetValue((length, code), out int run)) return run;
            }
            throw new PdfFilterException("CCITT data contains an invalid run code.");
        }
    }
    ...
    private ref struct BitReader
    {
        ...
        internal int Read()
        {
            if (_position >= _source.Length * 8)
                throw new PdfFilterException("CCITT data is truncated.");
            int bit = _source[_position / 8] >> (7 - _position % 8) & 1;
            _position++;
            return bit;
        }

        internal void Align() => _position = checked((_position + 7) / 8 * 8);

        internal void ReadEndOfLine()
        {
            int zeros = 0;
            while (Read() == 0) zeros++;
            if (zeros < 11)
                throw new PdfFilterException("A CCITT scan line has no valid end-of-line code.");
        }
    }
}
