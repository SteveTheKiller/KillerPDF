...
if (args.Length >= 3 && args[0] == "--ocr-train-corpus")
{
    ...
    int? minimumFormCharacterAccuracyPercent = null;
    int? maximumPageAllocationMegabytes = null;
    string? ocrFileListPath = null;
    string? ocrHoldoutFileListPath = null;
    ...
    for (int index = 3; index < args.Length; index += 2)
    {
        ...
        if (args[index] == "--holdout-file-list")
        {
            ocrHoldoutFileListPath = Path.GetFullPath(args[index + 1]);
            continue;
        }
        ...
        if (args[index] == "--language-model-input")
        {
            ocrLanguageModelInputPath = Path.GetFullPath(args[index + 1]);
            continue;
        }
        ...
        {
            ocrFontPaths.Add(Path.GetFullPath(args[index + 1]));
            continue;
        }
        if (!int.TryParse(args[index + 1], out int value) || value < 1)
        {
        ...
        switch (args[index])
        {
            ...
            case "--minimum-deskewed-character-accuracy-percent" when value <= 100:
                ...
                break;
            case "--minimum-form-character-accuracy-percent" when value <= 100:
                minimumFormCharacterAccuracyPercent = value;
                break;
            case "--maximum-page-allocation-megabytes":
                maximumPageAllocationMegabytes = value;
                break;
    ...
    if (ocrModelInputPath is not null
        && (!File.Exists(ocrModelInputPath)
            || string.Equals(modelPath, ocrModelInputPath,
                StringComparison.OrdinalIgnoreCase)))
    {
    ...
    if (ocrLanguageModelInputPath is not null
        && (!File.Exists(ocrLanguageModelInputPath)
            || string.Equals(ocrLanguageModelPath, ocrLanguageModelInputPath,
                StringComparison.OrdinalIgnoreCase)))
    {
        Console.Error.WriteLine(!File.Exists(ocrLanguageModelInputPath)
            ? $"OCR language-model input not found: {ocrLanguageModelInputPath}"
            : "OCR language-model input and output must be different files.");
        return 2;
    }
    ...
    TrueTypeFont[] ocrFonts;
    try
    {
        ocrFonts = [.. ocrFontPaths.Distinct(StringComparer.OrdinalIgnoreCase)
            .Select(LoadOcrFont)];
    }
    catch (Exception error) when (error is not OutOfMemoryException)
    {
        Console.Error.WriteLine($"OCR font input failed: "
            + $"{error.GetType().Name}: {error.Message}");
        return 2;
    }
    string[] ocrFiles;
    ...
    HashSet<string>? explicitHoldoutFiles = null;
    if (ocrHoldoutFileListPath is not null)
    {
        try
        {
            explicitHoldoutFiles = new HashSet<string>(
                ReadOcrFileList(ocrHoldoutFileListPath),
                StringComparer.OrdinalIgnoreCase);
        }
        catch (Exception error) when (error is not OutOfMemoryException)
        {
            Console.Error.WriteLine($"OCR holdout file list failed: {error.Message}");
            return 2;
        }
        var selectedFiles = new HashSet<string>(ocrFiles,
            StringComparer.OrdinalIgnoreCase);
        if (explicitHoldoutFiles.Count == 0
            || explicitHoldoutFiles.Count >= ocrFiles.Length
            || explicitHoldoutFiles.Any(file => !selectedFiles.Contains(file)))
        {
            Console.Error.WriteLine(
                "The OCR holdout file list must be a nonempty proper subset of the selected PDFs.");
            return 2;
        }
    }
    ...
    if (ocrHoldoutFileListPath is not null)
        Console.WriteLine($"OCR checkpoint holdout: {ocrHoldoutFileListPath}");
    foreach (string fontPath in ocrFontPaths.Distinct(StringComparer.OrdinalIgnoreCase))
    ...
    try
    {
        ...
        {
            using var input = new FileStream(ocrModelInputPath,
                FileMode.Open, FileAccess.Read, FileShare.Read);
            if (input.Length is <= 0 or > 256L * 1024 * 1024)
                throw new InvalidDataException("The OCR model input size is invalid.");
            var bytes = new byte[checked((int)input.Length)];
            input.ReadExactly(bytes);
            ocrModel = PdfOcrRecognitionModel.Load(bytes);
        }
    }
    catch (Exception error) when (error is not OutOfMemoryException)
    {
        Console.Error.WriteLine("OCR "
            + (ocrModelInputPath is null ? "training" : "model input")
            + $" failed: {error.GetType().Name}: {error.Message}");
        return 1;
    }
    Console.WriteLine($"OCR checkpoint: {ocrFiles.Length:N0} PDF files, "
        + $"{pagesPerFile:N0} page(s) per file, {renderSize:N0}-pixel render, "
        + $"{ocrModel.Width}x{ocrModel.Height} model, "
    ...
    if (ocrLanguageModelPath is not null || ocrLanguageModelInputPath is not null)
    {
        try
        {
            if (ocrLanguageModelInputPath is null)
            {
                ocrLanguageModel = PdfOcrLanguageModel.Train(LanguageTexts());
            }
            else
            {
                using var input = new FileStream(ocrLanguageModelInputPath,
                    FileMode.Open, FileAccess.Read, FileShare.Read);
                if (input.Length is <= 0 or > 64L * 1024 * 1024)
                    throw new InvalidDataException(
                        "The OCR language-model input size is invalid.");
                var bytes = new byte[checked((int)input.Length)];
                input.ReadExactly(bytes);
                ocrLanguageModel = PdfOcrLanguageModel.Load(bytes);
            }
            languageModelBytes = ocrLanguageModel.Save();
            Console.WriteLine($"OCR language model: {languageModelBytes.Length:N0} bytes, "
                + (ocrLanguageModelInputPath is null
                    ? "trained." : $"loaded from {ocrLanguageModelInputPath}."));
        }
        catch (Exception error) when (error is not OutOfMemoryException)
        {
    ...
    PdfOcrFormMetrics? formMetrics = null;
    long maximumPageAllocatedBytes = 0;
    if (minimumCharacterAccuracyPercent.HasValue
        ...
        || maximumPageAllocationMegabytes.HasValue)
    {
        try
        {
        ...
        {
            Console.Error.WriteLine($"OCR text evaluation failed: "
                + $"{error.GetType().Name}: {error.Message}");
            return 1;
        }
        ...
        if (maximumPageAllocationMegabytes.HasValue)
            Console.WriteLine($"OCR maximum page allocation: "
                + $"{maximumPageAllocatedBytes / 1024d / 1024d:N2} MiB.");
    }
    try
    {
        ...
        if (minimumFormCharacterAccuracyPercent.HasValue)
        {
            formMetrics = EvaluateHoldoutForms();
            Console.WriteLine($"OCR forms: {formMetrics.ExpectedFieldCount:N0} populated "
                + $"fields, {formMetrics.Text.CharacterAccuracy:P2} character accuracy.");
        }
    }
    catch (Exception error) when (error is not OutOfMemoryException)
    {
    ...
    if (maximumCalibrationErrorPercent.HasValue
        && evaluation.CalibrationError > maximumCalibrationErrorPercent.Value / 100d)
    {
    ...
    {
        Console.Error.WriteLine($"OCR model rejected: "
            + $"{textMetrics.CharacterAccuracy:P2} character accuracy is below the "
            + $"{minimumCharacterAccuracyPercent}% minimum.");
        return 1;
    }
    if (minimumWordAccuracyPercent.HasValue
        && textMetrics!.WordAccuracy < minimumWordAccuracyPercent.Value / 100d)
    {
    ...
    if (minimumFormCharacterAccuracyPercent.HasValue
        && formMetrics!.Text.CharacterAccuracy
            < minimumFormCharacterAccuracyPercent.Value / 100d)
    {
        Console.Error.WriteLine($"OCR model rejected: "
            + $"{formMetrics.Text.CharacterAccuracy:P2} form-field character accuracy "
            + $"is below the {minimumFormCharacterAccuracyPercent}% minimum.");
        return 1;
    }
    if (maximumPageAllocationMegabytes.HasValue
        && maximumPageAllocatedBytes
            > checked((long)maximumPageAllocationMegabytes.Value * 1024 * 1024))
    {
        Console.Error.WriteLine($"OCR model rejected: "
            + $"{maximumPageAllocatedBytes / 1024d / 1024d:N2} MiB maximum page "
            + $"allocation exceeds the {maximumPageAllocationMegabytes} MiB limit.");
        return 1;
    }
    ...
    IEnumerable<PdfOcrTrainingSample> Samples(string phase, bool selectHoldout)
    {
        ...
        for (int fileIndex = 0; fileIndex < ocrFiles.Length; fileIndex++)
        {
            ...
            bool isHoldout = IsOcrHoldout(file, relative);
    ...
    bool IsOcrHoldout(string file, string relative) =>
        explicitHoldoutFiles?.Contains(file)
            ?? PdfOcrTrainingPartition.IsHoldout(relative, holdoutPercent);

    static TrueTypeFont LoadOcrFont(string path)
    {
        if (!File.Exists(path))
            throw new FileNotFoundException("The OCR font file was not found.", path);
        var information = new FileInfo(path);
        if (information.Length is <= 0 or > 64L * 1024 * 1024)
            throw new InvalidDataException("The OCR font file size is invalid.");
        TrueTypeFont font = TrueTypeFont.Load(File.ReadAllBytes(path));
        if (!font.EmbeddingAllowed)
            throw new InvalidDataException("The OCR font does not permit embedding.");
        return font;
    }

    static bool FontSupportsLabel(TrueTypeFont font, string label)
    {
        Rune[] runes = [.. label.EnumerateRunes()];
        if (runes.Length == 1) return font.GetGlyphId(runes[0].Value) != 0;
        if (runes.Length != 2 || !IsVariationSelector(runes[1].Value)) return false;
        return font.GetGlyphId(runes[0].Value, runes[1].Value) != 0;

        static bool IsVariationSelector(int value) =>
    ...
    IEnumerable<string> LanguageTexts()
    {
        for (int fileIndex = 0; fileIndex < ocrFiles.Length; fileIndex++)
        {
            string file = ocrFiles[fileIndex];
            string relative = Path.GetRelativePath(ocrRoot, file);
            if (IsOcrHoldout(file, relative)) continue;
            string[] pages;
            try
            {
                using var timeout = new CancellationTokenSource(
        ...
        for (int fileIndex = 0; fileIndex < ocrFiles.Length; fileIndex++)
        {
            ...
            try
            {
                    ...
                    pageIndex < Math.Min(information.Count, pagesPerFile); pageIndex++)
                {
                    ...
                    recognizedCharacters = checked(recognizedCharacters
                        + pageMetrics.RecognizedCharacterCount);
                    characterEdits = checked(characterEdits + pageMetrics.CharacterEditCount);
                    ...
                    maximumPageAllocatedBytes = Math.Max(maximumPageAllocatedBytes,
            ...
            catch (Exception error) when (error is not OutOfMemoryException)
            {
                string detail = error is OperationCanceledException
                    ? $"exceeded {ocrTimeoutSeconds} seconds"
                    : $"{error.GetType().Name}: {error.Message}";
                Console.WriteLine($"SKIP  text {relative}: {detail}");
            }
        }
        if (pageCount == 0)
            throw new InvalidOperationException(
    ...
    PdfOcrFormMetrics EvaluateHoldoutForms()
    {
        int expectedFields = 0, recognizedFields = 0;
        int expectedCharacters = 0, recognizedCharacters = 0, characterEdits = 0;
        int expectedWords = 0, recognizedWords = 0, wordEdits = 0;
        var options = new PdfOcrOptions(["und"], deskew: true,
            correctOrientation: true, removeBackground: true, removeNoise: true,
            detectPageSegments: false);
        foreach (string file in ocrFiles)
        {
            string relative = Path.GetRelativePath(ocrRoot, file);
            if (!IsOcrHoldout(file, relative)) continue;
            try
            {
                    ...
                    pageIndex < Math.Min(information.Count, pagesPerFile); pageIndex++)
                {
                    ...
                    if (!widgets.Any(widget => widget.FieldKind is
                            PdfFormFieldKind.Text or PdfFormFieldKind.Choice
                    ...
                    PdfRenderedPage rendered = renderer.Render(pageIndex,
                        new PdfRenderOptions(width, height,
                            includeAnnotations: false, includeFormFields: true), timeout.Token);
                    PdfOcrResult recognized = PdfOcrFormRecognizer.Recognize(
                        RecognizeRaster, rendered.Pixels, rendered.Width, rendered.Height,
                        widgets, cancellationToken: timeout.Token);
                    PdfOcrFormMetrics pageMetrics = PdfOcrFormMetrics.Compare(
                        widgets, recognized, rendered.Width, rendered.Height);
                    expectedFields = checked(expectedFields + pageMetrics.ExpectedFieldCount);
                    recognizedFields = checked(recognizedFields
                        + pageMetrics.RecognizedFieldCount);
                    expectedCharacters = checked(expectedCharacters
                        + pageMetrics.Text.ExpectedCharacterCount);
                    recognizedCharacters = checked(recognizedCharacters
                        + pageMetrics.Text.RecognizedCharacterCount);
                    characterEdits = checked(characterEdits
                        + pageMetrics.Text.CharacterEditCount);
                    expectedWords = checked(expectedWords + pageMetrics.Text.ExpectedWordCount);
                    recognizedWords = checked(recognizedWords
                        + pageMetrics.Text.RecognizedWordCount);
                    wordEdits = checked(wordEdits + pageMetrics.Text.WordEditCount);
                }
            }
            catch (Exception error) when (error is not OutOfMemoryException)
            {
                string detail = error is OperationCanceledException
                    ? $"exceeded {ocrTimeoutSeconds} seconds"
                    : $"{error.GetType().Name}: {error.Message}";
                Console.WriteLine($"SKIP  forms {relative}: {detail}");
            }
        }
        if (expectedFields == 0)
            throw new InvalidOperationException(
                "The OCR holdout contains no populated text or choice fields.");
